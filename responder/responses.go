...
func WriteStatusAndBody(status int, body interface{}) {
	...
	case []byte:
		bodyProvider = func() io.ReadCloser { return io.NopCloser(bytes.NewBuffer(bdy)) }
	case string:
		bodyProvider = func() io.ReadCloser { return io.NopCloser(bytes.NewBufferString(bdy)) }
	case io.Reader:
		bodyProvider = func() io.ReadCloser { return io.NopCloser(bdy) }
	case func() io.Reader:
		bodyProvider = func() io.ReadCloser {
			r := bdy()
			if r == nil {
				return nil
			}
	...
	default:
		...
		bodyProvider = func() io.ReadCloser { return io.NopCloser(bytes.NewBuffer(bdyBytes)) }
